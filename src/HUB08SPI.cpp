...
void HUB08SPI::begin(uint8_t *displaybuf, uint16_t width, uint16_t height)
{
    ...
    this->width = width;
    this->height = height;
    DDRD |= 0xF0;                        // BIN = 0b11110000 ==> SET PIN 7,6,5,4 = OUTPUT (A,B,C,D)
    DDRB |= 0x03;                        // BIN = 0b00000011 ==> SET PIN 9 (PB1), 8 (PB0) = OUTPUT (OE, LAT)
    TCCR2B = TCCR2B & 0b11111000 | 0x01; // ~62kHz PWM on pin 11 (R1, if used)
    TCCR1B = TCCR1B & 0b11111000 | 0x01; // ~62kHz PWM on pin 9 (OE)
    SPI.begin();
    analogWrite(9, 128); // default brightness on OE (pin 9)
}
...
void HUB08SPI::scan()
{
    ...
    for (uint8_t line = 0; line < (height / 16); line++) // when more than 16 line display
    {
        ...
        for (uint8_t byte = 0; byte < (width / 8); byte++)
        {                       // CHANGE THIS IF DISPLAY INVERT
            SPI.transfer(*ptr); // SPI.transfer(*ptr);
            // SPI.transfer(~*ptr); // SPI.transfer(*ptr);
            ptr++;
        }
    }
    uint16_t r = -2000; // timeout. If something is wrong with the connection, we do not want to wait forever. This value is incremented each cycle of the blocking wait - while loop to bail out eventually, when OE never goes HIGH.
    while (!(PINB & (1 << 1)) && ++r)
        ;                                   // Wait for OE to go HIGH or Timeout (r == 0) [changed: PIND (1<<3) -> PINB (1<<1) for pin 9 / PB1]
    uint8_t t = (PIND & 0x0F) | (row << 4); // ?? Select row by binary? e.g. 1 -> A or 3 -> A,B or 5 -> A,C? yes, exactly. I shift row by 4 and or it to the output to make row appear on the upper half of Port D. the lower half must remain the same.
    PORTD = t;
    PINB = 1 << 0; // toggle latch. It's a lesser known feature of the atmega controllers. Writing to PINx toggles the output in only one processor cycle [changed: PIND (1<<2) -> PINB (1<<0) for pin 8 / PB0]
    PINB = 1 << 0;
    row = (row + 1) & 0x0F; // Everytime this function is called, it scans the next line. Lines only range from 0 to 15. "& 0xF" equals "% 16", so it wraps back to 0 after reaching 15.
}
