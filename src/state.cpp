...
bool triggerPressed() {
  const bool reading = (digitalRead(PIN_TRIGGER_BTN) == HIGH);

  if (reading != runtime.btnLastReading) {
    runtime.btnLastReading = reading;
    runtime.btnLastDebounceMs = millis();
    return false;  // baru berubah, tunggu stabil dulu
  }

  if (millis() - runtime.btnLastDebounceMs <= BTN_DEBOUNCE_MS) return false;
  if (reading == runtime.btnStableReading) return false;
...
bool timerTick() {
  if (runtime.state != TimerState::RUNNING || runtime.remainingMs == 0) return false;
